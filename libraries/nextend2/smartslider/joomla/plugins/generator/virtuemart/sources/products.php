...
class N2GeneratorVirtueMartProducts extends N2GeneratorAbstract {
    ...
    public function renderFields($form) {
        ...
        new N2ElementText($limit, 'fallbacklanguage', n2_('Fallback language'), '');
    ...
    protected function _getData($count, $startIndex) {
        ...
        $images = $model->db->queryAll($query);
        for ($i = 0; $i < count($data); $i++) {
            $k = 1;
            for ($j = 0; $j < count($images); $j++) {
                if ($data[$i]['id'] == $images[$j]['id']) {
                    $data[$i]['image_' . $k]     = N2JoomlaImageFallBack::fallback($root, array($images[$j]['file_url']));
                    $data[$i]['thumbnail_' . $k] = N2JoomlaImageFallBack::fallback($root, array(
