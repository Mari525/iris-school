...
class N2GeneratorJoomShoppingProducts extends N2GeneratorAbstract {
    ...
    public function renderFields($form) {
        ...
        new N2ElementOnOff($limit, 'allimage', n2_('Ask down all product images'), 0);
    ...
    protected function _getData($count, $startIndex) {
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            if ($allImage) {
                ...
                $images     = $model->db->queryAll($imageQuery);
                for ($j = 0; $j < count($images); $j++) {
                    $r += array(
                        'image' . ($j + 1)      => N2ImageHelper::dynamic($jShopConfig->image_product_live_path . '/' . $images[$j]['image_name']),
                        'thumbnail' . ($j + 1)  => N2ImageHelper::dynamic($jShopConfig->image_product_live_path . '/thumb_' . $images[$j]['image_name']),
                        'image_full' . ($j + 1) => N2ImageHelper::dynamic($jShopConfig->image_product_live_path . '/full_' . $images[$j]['image_name'])
                    );
                }
            }
