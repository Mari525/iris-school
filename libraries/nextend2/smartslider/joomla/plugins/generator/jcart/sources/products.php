...
if (!class_exists("Config")) require_once(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_opencart' . DIRECTORY_SEPARATOR . 'config.php');
if (!class_exists("Action")) require_once(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_opencart' . DIRECTORY_SEPARATOR . 'system' . DIRECTORY_SEPARATOR . 'startup.php');
if (!class_exists("ControllerStartupSeoUrl")) require_once(JPATH_ROOT . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_opencart' . DIRECTORY_SEPARATOR . 'catalog' . DIRECTORY_SEPARATOR . 'controller' . DIRECTORY_SEPARATOR . 'startup' . DIRECTORY_SEPARATOR . 'seo_url.php');
...
class N2GeneratorJCartProducts extends N2GeneratorAbstract {
    ...
    protected function _getData($count, $startIndex) {
        ...
        foreach ($result AS $res) {
            ...
            $r['thumbnail'] = $r['image'] = N2JoomlaImageFallBack::fallback($uri . "/", array(
                !empty($res['image']) ? JOOCART_RELATIVE_URL . 'image/' . $res['image'] : ''
            ), array(
                str_replace('../components/com_jcart', 'components/com_jcart', 'components/com_opencart', $res['description'])
            ));

            $baseUrl    = N2JoomlaImageFallBack::siteURL();
            $productUrl = $this->removeItemID($this->changeAnd($url->link('product/product', '&product_id=' . $res['product_id'])));
            $categoryUrl = $this->removeItemID($this->changeAnd($url->link('product/category', '&path=' . $res['category_id'])));

            $seo            = new N2ControllerStartupSeoUrl();
            $seoUrl         = $this->removeItemID($this->changeAnd(@$seo->rewrite($baseUrl . $productUrl)));
            $categorySeoUrl = $this->removeItemID($this->changeAnd(@$seo->rewrite($baseUrl . $categoryUrl)));
    ...
    private function removeItemID($url) {
        return preg_replace('~[?&]Itemid=[^&]*~', '$1', $url);
    }
