    ...
    foreach ($this->items as $i => $item) {
        ...
        <tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->id; ?>">
            ...
            <td class="center hidden-phone"><?php
                if ($canEdit || $canEditOwn) { ?>
                    <a href="<?php echo $linkFields; ?>" title="<?php echo JText::_('COM_VISFORMS_FIELDS_OPEN_LIST'); ?>"><?php echo $item->nbfields; ?></a><?php
                }
                else {
                    echo $item->nbfields;
                } ?>
			...
			<td class="center"><?php
                if ($canEdit || $canEditOwn) {
					if ($item->saveresult == '1') { ?>
                        <a href="<?php echo $linkDatas; ?>" title="<?php echo JText::_('COM_VISFORMS_PDFS_OPEN_LIST'); ?>"><?php echo $this->getDatasTotal($item->id); ?></a><?php
                    }
                } ?>
			</td><?php
            if($this->hasPdf) { ?>
