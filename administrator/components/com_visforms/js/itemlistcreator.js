...
(function ($) {
    ...
    $.extend($.fn, {
        ...
        storeVisformsOptionCreatorData() {
            var visformsOptionCreator = $.data(this[0], "visformsOptionCreator");
        ...
        var optionsTable = {
            createTable: function () {
                var html = '<div id="' + itemListContId + '">' +
                    ...
                    '<table id="' + itemListId + '" class="'+sortableClass+' table table-striped table-condensed"><thead>' + settings.params.header + '</thead><tbody>' + settings.params.items + '</tbody></table></div>';
                $(html).insertBefore('#' + dbFieldName);
                $('#' + itemListContId).trigger("subform-row-add");
            },
