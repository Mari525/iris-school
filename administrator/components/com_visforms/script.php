...
class com_visformsInstallerScript
{
	...
	public function preflight($route, $adapter) {
		...
		$this->versionsWithPostflightFunction = array('3.1.0', '3.2.0', '3.3.0', '3.4.0', '3.4.1', '3.5.1', '3.6.0', '3.6.3', '3.6.5', '3.7.0', '3.8.17', '3.10.0', '3.10.1', '3.10.2', '3.12.0');
		...
		if ($route != 'uninstall') {
            if( version_compare( $jversion->getShortVersion(), $this->minimum_joomla_release, 'lt' ) ) {
                ...
                $this->addVisformsLogEntry($msg, \JLog::ERROR, 'jerror');
                return false;
            }
			if (property_exists( 'JVersion', 'MAJOR_VERSION') && $jversion::MAJOR_VERSION > $this->maximum_joomla_release) {
	...
	private function warnSubUpdateRequired ($route){
		...
		try {
			...
			if (!empty($version) && version_compare($version, $this->vfsubminversion, 'lt')) {
				$msg = '<br /><strong style="color: red;">' . \JText::sprintf('COM_VISORMS_SUBSCRIPTION_UPDATE_REQUIRED', $this->vfsubminversion) . '</strong>';
				$this->status->component = array('name' => 'visForms', 'type' => $route, 'msg' => $msg);
			}
		}
		catch (Exception $e) {
