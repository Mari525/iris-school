    ...
    public static function extractHiddenList ($optionString = '')
    {
        ...
        if ($optionString != "")
        {
            ...
            foreach ($options as $option)
            {
                ...
                $option->listitemredirecturl = (isset($option->listitemredirecturl)) ? $option->listitemredirecturl : '';
	            $option->listitemmail = (isset($option->listitemmail)) ? $option->listitemmail : '';
	            $option->listitemmailcc = (isset($option->listitemmailcc)) ? $option->listitemmailcc : '';
	            $option->listitemmailbcc = (isset($option->listitemmailbcc)) ? $option->listitemmailbcc : '';

                $returnopts[] = array( 'id' => $option->listitemid, 'value' => $option->listitemvalue, 'label' => $option->listitemlabel, 'selected' => $selected, 'redirecturl' => $option->listitemredirecturl, 'mail' => $option->listitemmail, 'mailcc' => $option->listitemmailcc, 'mailbcc' => $option->listitemmailbcc);
            }
        }       
        return $returnopts;
    }
