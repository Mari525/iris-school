...
class VisformsModelVisdatas extends JModelList
{
    ...
    public function createExportBuffer ($params = null, $cIds = array()) {
        ...
        $separator = (isset($params->expseparator)) ? $params->expseparator : ";";
        $csvHelper = new visFormCsvHelper($params);
        ...
        if ((!(isset($params->includeheadline))) || ((isset($params->includeheadline)) && ($params->includeheadline == 1))) {
            if (!empty($params->expfieldid)) {
	            $lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_ID' ));
            }
            if (!empty($params->expfieldpublished)) {
	            $lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_PUBLISHED' ));
            }
            if (!empty($params->expfieldcreated)) {
	            $lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_FIELD_CREATED_LABEL' ));
            }
            if (!empty($params->expfieldcreatedby)) {
	            $lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_FIELD_CREATED_BY_LABEL' ));
            }
            ...
            if (!empty($params->expfieldip)) {
	            $lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_IP' ));
            }
            if (!empty($params->expfieldismfd)) {
	            $lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_MODIFIED' ));
            }
			if (!empty($params->expfieldmodifiedat)) {
				$lineBuffer[] = $csvHelper->convertCharacterSet(JText::_( 'COM_VISFORMS_MODIFIED_AT' ));
			}
