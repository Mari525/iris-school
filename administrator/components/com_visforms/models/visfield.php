...
class VisformsModelVisfield extends VisFormsItemModelBase
{
	...
	public function getForm($data = array(), $loadData = true) {
		...
		if ($fid != 0) {
			...
			if (empty($hasBt4) || (!empty($visform) && ($visform->layoutsettings['formlayout'] != 'bt4mcindividual'))) {
			...
			if (empty($hasBt4) || ((!empty($visform) && (!isset($visform->layoutsettings['displaysublayout']) || ($visform->layoutsettings['displaysublayout'] != 'individual'))))) {
				$form->removeGroup('gridSizes');
			}
		}
		$user = JFactory::getUser();
