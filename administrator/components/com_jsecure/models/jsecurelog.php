...
function getBlockedCountry(){
	...
	function changedbprefix($table_new_prefix){
		...
		if (is_array($result) && count($result) > 0){
            ...
            {
	        $flag = 0;
			$db1->transactionStart();
			$rename_query = rtrim($rename_query,',');
			$db1->setQuery($rename_query);
			$db1->query();
			$db1->transactionCommit();
            }			
		    catch (Exception $e)
           {
			// catch any database errors.
           $db1->transactionRollback();
	...
	function saveBlackIp($bips){
		$db = JFactory::getDbo();
		$query ="insert into #__jsecure_blackiplist(iptype,ip) VALUES('0','".$bips."')";
		$db->setQuery($query);
		$db->query();	
	}
	
	function getBlackIps(){
		$db = JFactory::getDbo();
		$query ="select DISTINCT ip from #__jsecure_blackiplist";
		$db->setQuery($query);
		$result = $db->loadObjectList();
		return $result;	
	}
	...
	function emptyIpList(){
		$db = JFactory::getDbo();
		$query ="TRUNCATE TABLE #__jsecure_blackiplist";
		$db->setQuery($query);
		$db->query();	
	}
	
	function saveWhiteIp($bips){
		$db = JFactory::getDbo();
		$query ="insert into #__jsecure_whiteiplist(iptype,ip) VALUES('1','".$bips."')";
		$db->setQuery($query);
		$db->query();	
	}
	
	function getWhiteIps(){
		$db = JFactory::getDbo();
		$query ="select DISTINCT ip from #__jsecure_whiteiplist";
		$db->setQuery($query);
		$result = $db->loadObjectList();
		return $result;	
	}
	
	function emptyIpWhiteList(){
		$db = JFactory::getDbo();
		$query ="TRUNCATE TABLE #__jsecure_whiteiplist";
		$db->setQuery($query);
		$db->query();	
	}
	
	function existInWhiteList($ip){
	
	  $db =JFactory::getDBO();
	  $query ="SELECT ip from #__jsecure_whiteiplist where ip='".$ip."'";
	  $db->setQuery($query);
	  $result =$db->loadObjectList();
	  return $result;
	}
	
	function existInBlackList($ip){
	
	  $db =JFactory::getDBO();
	  $query ="SELECT ip from #__jsecure_blackiplist where ip='".$ip."'";
	  $db->setQuery($query);
	  $result =$db->loadObjectList();
	  return $result;
	}
	...
	$rows =$db->loadRow();
	if(empty($rows))
	$query ="INSERT INTO #__jsecure_license (license_key) VALUES ('".$license_key."')";
	else
	$query ="UPDATE #__jsecure_license SET license_key = '".$license_key."'";
	$db->setQuery($query);
	$result = $db->query();	
	return true;
	}
