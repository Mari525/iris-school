    ...
    protected function setCustomJs() {
	    ...
	    switch ($this->form->formlayout) {
		    case 'bt4mcindividual' :
			    $script = '
			        jQuery(document).ready( function() {
				        ...
				        jQuery("<span class=\"input-group-btn\"><btn type=\"button\" class=\"btn btn-secondary verifyMailBtn\" onclick=\"verifyMail(\'field'.$field->id.'\',\''.$field->fid.'\',\''.JSession::getFormToken().'\'); return false;\">'. JText::_('COM_VISFORMS_VERIFY') .'</btn></div>").insertAfter("#field'.$field->id.'");
		    ...
		    case 'bt3horizontal' :
			    $script = '
			        jQuery(document).ready( function() {	        
				        jQuery("<div class=\"fc-tbxfield' . $field->id . '_code\"></div><div class=\"input-group\"><span class=\"btn input-group-addon verifyMailBtn\" onclick=\"verifyMail(\'field' . $field->id . '\',\'' . $field->fid . '\',\'' . JSession::getFormToken() . '\'); return false;\">' . JText::_('COM_VISFORMS_VERIFY') . '</span><input type=\"text\" id=\"field' . $field->id . '_code\" name=\"' . $field->name . '_code\" class=\"form-control verificationCode' . $extraClass . '\"' . $extraAttribs . ' /></div>").insertAfter("#field' . $field->id . '");
				        jQuery("#field' . $field->id . '").on("change", function () {
				            if (jQuery(this).val()) {
				                jQuery("#field' . $field->id . '_code").prop("required", true);
				            }
		    ...
		    case 'bt3mcindividual' :
			    $script = '
			        jQuery(document).ready( function() {	        
				        jQuery("<div class=\"input-group\"><span class=\"btn input-group-addon verifyMailBtn\" onclick=\"verifyMail(\'field' . $field->id . '\',\'' . $field->fid . '\',\'' . JSession::getFormToken() . '\'); return false;\">' . JText::_('COM_VISFORMS_VERIFY') . '</span><input type=\"text\" id=\"field' . $field->id . '_code\" name=\"' . $field->name . '_code\" class=\"form-control verificationCode' . $extraClass . '\"' . $extraAttribs . ' /></div><div class=\"fc-tbxfield' . $field->id . '_code\"></div>").insertAfter("#field' . $field->id . '");
				        jQuery("#field' . $field->id . '").on("change", function () {
				            if (jQuery(this).val()) {
				                jQuery("#field' . $field->id . '_code").prop("required", true);
				            }
