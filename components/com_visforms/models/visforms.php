...
class VisformsModelVisforms extends JModelLegacy
{
	...
	public function getForm() {
		...
		if (empty($storedFormIsValid)) {
			...
			$form->layoutsettings = VisformsHelper::registryArrayFromString($form->layoutsettings);
			...
			//check if bootstrap 4 layouts are installed, if not reset to bootstrap 2.3.2 layouts
			if (empty($this->hasBt4Layouts)) {
			...
			$form->captchaoptions = VisformsHelper::registryArrayFromString($form->captchaoptions);
			foreach ($form->captchaoptions as $name => $value) {
				//make names shorter and set all captchaoptions as properties of form object
				$form->$name = $value;
			}
			$form->viscaptchaoptions = VisformsHelper::registryArrayFromString($form->viscaptchaoptions);
			...
			//enforce bt4 property to be set
			if ($form->formlayout === 'bt4mcindividual') {
				if (empty($form->captchaLabelBootstrapWidth) || ($form->displaysublayout != 'individual')) {
					if ($form->displaysublayout == 'stacked' || $form->displaysublayout == 'individual') {
						$form->captchaLabelBootstrapWidth = "12";
					}
					else {
						$form->captchaLabelBootstrapWidth = "3";
					}
				}
				foreach ($this->breakPoints as $breakPoint) {
					$name = 'captchaLabelBootstrapWidth' . $breakPoint;
					if (empty($form->$name) || ($form->displaysublayout != 'individual')) {
						if ($form->displaysublayout == 'stacked' || $form->displaysublayout == 'individual') {
							$form->$name = "12";
						}
						else {
				...
				$form->ctrlGroupBtClasses = $this->getCtrlGroupBtClasses($form);
				$form->indentedBtClasses = $this->getIndentedBtClasses($form);
				$form->captchaLabelClasses = $this->getCaptchaLabelClasses($form);
				if (empty($form->summarybtncssclass)) {
					$form->summarybtncssclass = 'btn-info';
				}
				if (empty($form->correctbtncssclass)) {
					$form->correctbtncssclass = 'btn-info';
				}
				if (empty($form->backbtncssclass)) {
					$form->backbtncssclass = 'btn-info';
				}
				if (empty($form->savebtncssclass)) {
					$form->savebtncssclass = 'btn-primary';
				}
				if (empty($form->cancelbtncssclass)) {
					$form->cancelbtncssclass = 'btn-secondary';
				}
			}
			else {
				$form->displaysublayout = 'horizontal';
	...
	protected function getCtrlGroupBtClasses($form) {
		$classes = (!empty($form->showcaptchalabel) && $form->captchaLabelBootstrapWidth != "12") ? 'offset-' . $form->captchaLabelBootstrapWidth . ' col-' . (12 - $form->captchaLabelBootstrapWidth) : (($form->captchaLabelBootstrapWidth != "12") ? ' col-' . (12 - $form->captchaLabelBootstrapWidth) : ' col-12');
		foreach ($this->breakPoints as $breakPoint) {
			$name = 'captchaLabelBootstrapWidth' . $breakPoint;
			$lcBreakPoint = lcfirst($breakPoint);
			$classes .= (!empty($form->showcaptchalabel) && $form->$name != "12") ? ' offset-' . $lcBreakPoint . '-' . $form->$name . ' col-' . $lcBreakPoint . '-' . (12 - $form->$name) : ((empty($form->showcaptchalabel) && $form->$name != "12") ? ' col-' . $lcBreakPoint . '-' . (12 - $form->$name) : '');
		}
		return $classes;
	}

	// use for indentation of error div
	protected function getIndentedBtClasses($form) {
		$indentedBtClasses = ($form->captchaLabelBootstrapWidth != "12") ? 'offset-' . $form->captchaLabelBootstrapWidth . ' col-' . (12 - $form->captchaLabelBootstrapWidth) : 'col-12';
		foreach ($this->breakPoints as $breakPoint) {
			$name = 'captchaLabelBootstrapWidth' . $breakPoint;
			$lcBreakPoint = lcfirst($breakPoint);
			$indentedBtClasses .= ($form->$name != "12") ? ' offset-' . $lcBreakPoint . '-' . $form->$name . ' col-' . $lcBreakPoint . '-' . (12 - $form->$name) : '';
		}
		return $indentedBtClasses;
	}

	protected function getCaptchaLabelClasses($form) {
		$labelClasses = 'col-' . $form->captchaLabelBootstrapWidth;
		foreach ($this->breakPoints as $breakPoint) {
			$name = 'captchaLabelBootstrapWidth' . $breakPoint;
			$lcBreakPoint = lcfirst($breakPoint);
			$labelClasses .= ($form->$name != "12") ? ' col-' . $lcBreakPoint . '-' . $form->$name : '';
		}
		$labelClasses .= (!empty($form->showcaptchalabel)) ? ' col-form-label sr-only' : ' col-form-label';
