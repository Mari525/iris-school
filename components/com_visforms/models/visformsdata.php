...
class VisformsModelVisformsdata extends JModelList
{
	...
	protected function cleanForm($form) {
		...
		$displayParameters = array ('displayip', 'displayid', 'displaycreated', 'displaycreatedtime', 'displayismfd', 'displaymodifiedat', 'displaymodifiedattime');
	...
	protected function getFilter() {
		...
		if (($searchfilter != '') && (!empty($fields))) {
			...
			foreach ($fields as $field) {
				...
				if (!in_array($field->typefield, array('select', 'radio', 'multicheckbox', 'signature'))) {
					$prop = "F" . $field->id;
	...
	protected function getListBoxFilterField($field) {
		...
		$options = '<option value="">' . JText::sprintf('COM_VISFORMS_FILTER_SELECT_SELECT_A_VALUE', $field->label) . '</option>';
