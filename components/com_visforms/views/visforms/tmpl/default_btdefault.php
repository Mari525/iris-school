...
<form action="<?php echo JRoute::_($this->formLink) ; ?>" method="post" name="visform" 
	...
	class="visform <?php echo $this->visforms->formlayout; ?> <?php echo $this->visforms->formCSSclass; echo (($this->visforms->formlayout == "bthorizontal") || ($this->visforms->formlayout == "bt3horizontal")) ? " form-horizontal " : "";?>"<?php if($this->upload == true) { ?> enctype="multipart/form-data"<?php } ?> role="form"> <?php
//add a progressbar
    ...
    if ($f === $this->steps) {
        ...
        if (empty($this->visforms->displaysummarypage)) {
            echo JLayoutHelper::render('visforms.footers.btdefault.nosummary', array('form' => $this->visforms, 'nbFields' => $this->nbFields, 'hasRequired' => $this->required));
        } //with summary page
        else {
            echo JLayoutHelper::render('visforms.footers.btdefault.withsummary', array('form' => $this->visforms, 'nbFields' => $this->nbFields, 'hasRequired' => $this->required, 'summarypageid' => $this->visforms->parentFormId));
        }
    }
    echo '</fieldset>';
    } ?>
