...
<form action="<?php echo htmlspecialchars($formLink, ENT_COMPAT, 'UTF-8'); ?>" method="post" name="visform"
    ...
    if ($f === $steps) {
        //no summary page
			if (empty($visforms->displaysummarypage)) {
				echo JLayoutHelper::render('visforms.footers.btdefault.nosummary', array('form' => $visforms, 'nbFields' => $nbFields, 'hasRequired' => $required), null, array('component' => 'com_visforms'));
			} //with summary page
			else {
				echo JLayoutHelper::render('visforms.footers.btdefault.withsummary', array('form' => $visforms, 'nbFields' => $nbFields, 'hasRequired' => $required, 'summarypageid' => $visforms->parentFormId), null, array('component' => 'com_visforms'));
			}
		}
	echo '</fieldset>';
	} ?>
